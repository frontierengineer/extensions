...
## 1. Put your application in a public GitHub repo
...
Repo layout = application layout (the same tree Frontier runs from
`applications/<id>/`):

```
application.json      required: {"displayName": "...", "defaultColor": "#rrggbb", "description": "..."}
...
- **Prefer zero dependencies.** Frontier installs third-party application deps
  with `npm install --ignore-scripts`, so packages relying on install scripts
  will not work. Vendor what you can.
- **Declare your network needs.** If your application calls out to the internet,
...
`<name>` is the application id users see (lowercase, hyphens). It doesn't have
...
in `.tgz` or `.tar.gz` — a tarball of the application tree with `application.json`
at its root:

```sh
tar -czf /tmp/application.tgz --exclude .git --exclude .github --exclude node_modules --exclude data .
gh release create v1.0.0 /tmp/application.tgz --title "v1.0.0"
```
...
jobs:
  release:
    ...
    steps:
      ...
      - run: tar -czf /tmp/application.tgz --exclude .git --exclude .github --exclude node_modules --exclude data .
      - run: gh release create "$GITHUB_REF_NAME" /tmp/application.tgz --title "$GITHUB_REF_NAME"
        env:
...
reason is recorded in your application's `rejected` list in the index.
...
- **Unpublish** by removing your `listings/<owner>/<name>.json`. On the next
  ...
  application is gone from `index.json` and stops appearing in the Marketplace
  tab — the indexer only emits entries for applications still listed. Your repo
...
- **Republish** by restoring the listing file (`{"repo": "<owner>/<repo>"}`,
  ...
  application reappears with its versions. Nothing about the releases needs to
  change — the listing coming back is enough.
- **Pull a single bad version.** Unpublishing and republishing is a whole-
  application switch; the index never *un*-pins a version it already accepted
  ...
  [`blocklist.json`](blocklist.json): an admin adds the application id (with a
  reason), instances poll it, and the application is barred from install and
...
VSIX), and any installed application can be exported as a tarball from the
Applications view. Hand someone the file if you like.
